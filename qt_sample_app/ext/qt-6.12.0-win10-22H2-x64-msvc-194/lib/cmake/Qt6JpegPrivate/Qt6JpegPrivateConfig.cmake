...
include("${CMAKE_CURRENT_LIST_DIR}/Qt6JpegPrivateTargetsPrecheck.cmake")

if(NOT __qt_JpegPrivate_skip_include_targets_file AND Qt6JpegPrivate_FOUND)
    ...
    include("${CMAKE_CURRENT_LIST_DIR}/Qt6JpegPrivateExtraProperties.cmake"
        OPTIONAL)
    set(__qt_JpegPrivate_targets_file_included ON)
endif()

# Find the private module counterpart.
set(__qt_JpegPrivate_always_load_private_module OFF)
include("${CMAKE_CURRENT_LIST_DIR}/Qt6JpegPrivate-build.cmake" OPTIONAL)
if (Qt6JpegPrivate_FOUND
        AND NOT Qt6JpegPrivatePrivate_FOUND
        AND NOT TRUE
        AND (
            __qt_JpegPrivate_always_load_private_module
    ...
    find_package(Qt6JpegPrivatePrivate "6.12.0" EXACT
        QUIET
        CONFIG
        PATHS
    ...
    if(NOT Qt6JpegPrivatePrivate_FOUND)
        get_property(Qt6JpegPrivatePrivate_warning_shown GLOBAL PROPERTY
            Qt6JpegPrivatePrivate_warning_shown
        )
        if(NOT Qt6JpegPrivatePrivate_warning_shown)
            message(VERBOSE
                "The private module package 'Qt6JpegPrivatePrivate' "
                "could not be found. It possibly needs to be installed separately with your "
                "package manager."
            )
            set_property(GLOBAL PROPERTY
                Qt6JpegPrivatePrivate_warning_shown ON
            )
        endif()
    endif()
endif()
unset(__qt_JpegPrivate_always_load_private_module)

if(__qt_JpegPrivate_targets_file_included)
    # DEPRECATED
    ...
    if(NOT Qt6JpegPrivate_DEFINITIONS)
        ...
        set(updated_defs "")
        foreach(def IN LISTS  Qt6JpegPrivate_DEFINITIONS)
        ...
        endforeach()
        set(Qt6JpegPrivate_DEFINITIONS "${updated_defs}")
        unset(updated_defs)
    endif()
    ...
    foreach(_module_dep ${_Qt6JpegPrivate_MODULE_DEPENDENCIES})
        ...
        list(APPEND Qt6JpegPrivate${_private_suffix}_INCLUDE_DIRS
             ${Qt6${_module_dep}_INCLUDE_DIRS})
        list(APPEND Qt6JpegPrivate${_private_suffix}_PRIVATE_INCLUDE_DIRS
             ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        if(_private_suffix)
            list(APPEND Qt6JpegPrivate_PRIVATE_INCLUDE_DIRS
                ${Qt6${_module_dep}_PRIVATE_INCLUDE_DIRS})
        endif()
        list(APPEND Qt6JpegPrivate${_private_suffix}_DEFINITIONS
             ${Qt6${_module_dep}_DEFINITIONS})
        list(APPEND Qt6JpegPrivate${_private_suffix}_COMPILE_DEFINITIONS
...
if (TARGET Qt6::JpegPrivate)
    ...
    foreach(extra_cmake_include Qt6JpegPrivateTransitiveExtras.cmake)
    ...
    if(NOT "JpegPrivate" IN_LIST QT_ALL_MODULES_FOUND_VIA_FIND_PACKAGE)
    ...
    if(NOT QT_NO_CREATE_VERSIONLESS_TARGETS)
        if(CMAKE_VERSION VERSION_LESS 3.18 OR QT_USE_OLD_VERSION_LESS_TARGETS)
            ...
            include("${CMAKE_CURRENT_LIST_DIR}/Qt6JpegPrivateVersionlessAliasTargets.cmake")
        endif()
    endif()
else()
    ...
    if(NOT DEFINED Qt6JpegPrivate_NOT_FOUND_MESSAGE)
        ...
        if(__qt_JpegPrivate_skip_include_targets_file)
            string(APPEND Qt6JpegPrivate_NOT_FOUND_MESSAGE
                "Possibly due to __qt_JpegPrivate_skip_include_targets_file being set to TRUE and thus "
