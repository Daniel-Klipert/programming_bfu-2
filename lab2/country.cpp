...
Country Country::operator*(const Country& other) {
    ...
    while (true) {
        ...
        if (other.cities_.at(r) != other.capital_) {
            for (int i = 0; i < other.cities_.size(); ++i) {
                bool has_city = false;
                
                for (int j = 0; j < rcities.size(); ++j) {
                    if (other.cities_.at(r) == rcities.at(j)) {
                        has_city = true;
                        break;
                    }
                }
                if (!has_city) {
                    ...
                    rcities.push_back(other.cities_.at(r) + "новый");
                    break;
                }
            }
...
void Country::set_capital(const string& capital) {
    ...
    if (allowed) {
        ...
        for (int i = 0; i < cities_.size(); ++i) {
            if (cities_.at(i) == capital) {
                allowed = false;
                break;
            }
        }
        if (allowed) {
            vector<string> ncities = cities_;
            ncities.insert(ncities.begin(), capital);
        }
    }
